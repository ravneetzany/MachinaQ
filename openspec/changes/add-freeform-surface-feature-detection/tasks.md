...
- [x] 7.1 Fix `freeform` primitives having no position data at all (`face_correlation.py` skips any primitive missing `axis_px`/`py`/`pz` entirely, so `freeform` primitives were structurally invisible to the correlation search regardless of proximity): add `StepTextParser.get_face_centroid()` and store it as point-only position data (no direction) in `pipeline.py`'s `freeform` branch. See design.md's "Post-implementation correction" section.
  ...
  - Re-verified against the real fixture: `freeform` primitives now carry real position data, confirmed not None. Noted remaining limitation (vertex-average centroid can still be far from a specific click point on a large patch) in design.md — flagged as a possible follow-up.
- [x] 7.2 Improve correlation precision for large `freeform` patches (user chose this over leaving 7.1's noted limitation as-is): add `StepTextParser.get_face_boundary_points()` (deduplicated boundary vertices) and store as `details["boundary_points"]` for `freeform` primitives; `face_correlation.py`'s `nearest_primitive()` now takes the minimum over the centroid and every boundary point when present. See design.md's updated "Post-implementation correction" section.
  - Added `test_face_boundary_points_deduplicated` (`tests/test_parser_geometry.py`), extended `test_freeform_primitive_carries_extents_when_resolvable` for `boundary_points` (`tests/test_pipeline.py`), and `test_nearest_primitive_uses_nearest_boundary_point_not_just_centroid` / `test_nearest_primitive_without_boundary_points_is_unaffected` (`tests/test_freecad_face_correlation.py`). Full suite: 110/110 passing.
  - Re-verified against the real fixture: the click point that previously matched a conical primitive 4.63mm away now matches the actual `freeform` primitive at 3.29mm.
