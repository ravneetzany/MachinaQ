...
- **BRepNet**: primarily a topological message-passing architecture (operates directly on B-Rep coedges/wingedges rather than a generic face-adjacency graph) — documented as a representation technique, not a new independent class table. **Correction found during primary-source verification:** design intended to say BRepNet was evaluated against the MFCAD-family taxonomy; the actual paper (verified via its arXiv HTML rendering) introduces and evaluates exclusively on its own Fusion 360 Gallery segmentation dataset (8 modeling-operation classes — ExtrudeSide/End, CutSide/End, Fillet, Chamfer, RevolveSide/End), with MFCAD cited only in Related Work, never used in BRepNet's own experiments. The mapping document records the corrected finding.
