...
echo ============================================================
echo  AAGNet Setup for Machining Feature Recognition
...
REM ── 1. Clone AAGNet ──────────────────────────────────────────
if not exist "aagnet\" (
    echo [1/3] Cloning AAGNet...
    git clone https://github.com/whjdark/AAGNet.git aagnet
    if errorlevel 1 (
        echo [ERROR] git clone failed. Is git installed and on PATH?
        exit /b 1
    )
    echo [OK]  aagnet/ cloned
) else (
    echo [OK]  aagnet/ already present -- skipping clone
)
...
if errorlevel 1 (
    echo.
    echo [2/3] Creating pyocc conda environment from aagnet/environment.yaml
    echo       This downloads ~3 GB of packages and takes 10-20 minutes...
    conda env create -f aagnet\environment.yaml -n pyocc
    if errorlevel 1 (
...
echo.
echo  1. Open  aagnet\README.md  and find the Google Drive link
...
echo.
echo  3. Train:
echo       conda run -n pyocc python train_aagnet.py
echo.
echo  Optional flags:
